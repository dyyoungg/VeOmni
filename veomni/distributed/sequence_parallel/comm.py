...
# ------------------------- Ulysses CPU (Gloo) Group ------------------------ #
def get_ulysses_sequence_parallel_cpu_group() -> Optional[dist.ProcessGroup]:
    """CPU (gloo) mirror of the Ulysses SP group of the current parallel state.

    Used by the dataloader (e.g. ``ulysess_dataloader``) to verify data
    consistency across SP ranks without touching GPU resources. The group is
    built in ``ParallelState.__post_init__`` when SP is enabled with a device
    mesh, and — like ``ulysses_group`` — is scoped per-module by
    ``use_parallel_state``. Returns ``None`` when SP is disabled or the current
    state has no CPU group attached.
    """
    ps = _current_state()
    return ps.ulysses_cpu_group if ps is not None else None
